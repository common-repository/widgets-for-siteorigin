...
function wpinked_so_post_byline( $byline, $id, $sep_cats, $sep_tags, $date = '' ) {
	...
	$byline = str_replace( '%author%', '<a href="' . get_author_posts_url( get_post_field( 'post_author', $id ) ) . '">' . get_the_author() . '</a>', $byline );
...
if ( ! function_exists ( 'wpinked_so_cf7_list' ) ) :
	function wpinked_so_cf7_list() {

		$args = array('post_type' => 'wpcf7_contact_form', 'posts_per_page' => -1);
		$forms = array();
		if( $data = get_posts($args) ){
			$forms['0'] = __( 'Select', 'wpinked-widgets' );
			foreach($data as $key){
...
// Returns icon to used in sliders.
function wpinked_get_so_icon($icon, $icon_styles) {
	return trim( preg_replace( '/\s+>/', '>', siteorigin_widget_get_icon( $icon, $icon_styles ) ) );
}